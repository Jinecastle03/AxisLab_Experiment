...
public class YawGaussian : MonoBehaviour
{
    ...
    /// <summary>
    /// Starts the yaw haptics loop. speedDegPerSec can be negative (direction reversal).
    /// </summary>
    ...
    // ---- Extra setters so Manager/Inspector can control yaw parameters ----
    public void SetCutoff01(float v) => cutoff01 = Mathf.Clamp(v, 0f, 0.4f);
    public float GetCutoff01() => cutoff01;

    public void SetPerceptualThreshold01(float v) => perceptualThreshold01 = Mathf.Clamp(v, 0f, 0.5f);
    public float GetPerceptualThreshold01() => perceptualThreshold01;

    public void SetMinOn01(float v) => minOn01 = Mathf.Clamp(v, 0f, 0.2f);
    public float GetMinOn01() => minOn01;

    public void SetSmoothingTau(float v) => smoothingTau = Mathf.Max(1e-4f, v);
    public float GetSmoothingTau() => smoothingTau;

    public void SetDurationMillis(int ms) => durationMillis = Mathf.Max(10, ms);
    public int GetDurationMillis() => durationMillis;
